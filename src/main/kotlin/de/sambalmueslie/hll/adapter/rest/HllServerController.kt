...
class HllServerController(private val service: HllServerService) : HllServerAPI {
    ...
    @Get("/{serverId}/name")
    override fun getName(auth: Authentication, @PathVariable serverId: Long) = service.getServerName(auth, serverId)

    @Get("/{serverId}/slots")
    override fun getSlots(auth: Authentication, @PathVariable serverId: Long) = service.getServerSlots(auth, serverId)
